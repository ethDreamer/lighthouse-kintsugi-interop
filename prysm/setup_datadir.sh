...
mkdir -p $DATADIR/network && \
	cd $DATADIR/network && \
	cp $PRYSM_DIR/../genesis/eth2_config.yaml ./config.yaml && \
	...
	cp $PRYSM_DIR/../genesis/eth2/public/genesis.ssz . && \
	cd - 1>/dev/null
...
for full in $(echo $PRYSM_DIR/../genesis/eth2/private/*); do
	base=$(basename $full);
	mkdir $base && \
		cd $base && \
		cp -r $full/prysm ./ && \
